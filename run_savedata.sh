...
if [ -f "$SYMBOLS_FILE" ]; then
    ...
    SYMBOL_DATE=$(python3.11 -c "import json; print(json.load(open('$SYMBOLS_FILE'))['date'])" 2>/dev/null)
    TODAY=$(date '+%Y-%m-%d')

    if [ "$SYMBOL_DATE" = "$TODAY" ]; then
        SYMBOL_COUNT=$(python3.11 -c "import json; print(json.load(open('$SYMBOLS_FILE'))['total_symbols'])" 2>/dev/null)
